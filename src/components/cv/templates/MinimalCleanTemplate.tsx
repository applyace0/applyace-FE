...
export function MinimalCleanTemplate({ data }: { data: any }) {
  ...
  return (
    <div className="bg-white p-8 rounded shadow text-gray-900 max-w-2xl mx-auto space-y-6 font-[Inter] max-h-[280mm] overflow-hidden print:max-h-none">
        ...
        <div className="border-t border-gray-200 pt-4 mt-4">
          ...
          <ul className="list-disc pl-8 space-y-3 text-left">
            {data.references.map((ref: any, idx: number) => (
              <li key={idx}>
                <strong>{ref.name}</strong>, {ref.title} at {ref.company}<br />
                {ref.email} | {ref.phone}
              </li>
            ))}
          </ul>
